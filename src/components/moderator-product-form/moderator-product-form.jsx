...
const ModeratorProductFormContainer = ({ className, product, setProducts, isAdding }) => {
	...
	const handleSubmit = (event) => {
		...
		const productStatus = formData.get('status')?.toString() || 'regular';
		const stock = productStatus === 'out' ? 0 : 1;
		const isFeatured = productStatus === 'featured';
				...
				currentProducts.map((currentProduct) =>
					currentProduct.id === product.id
						? {
								...
								stock,
								description,
								images: [imageUrl, ...currentProduct.images.slice(1)],
							}
						: currentProduct,
				),
			);
	...
	if (!isAdding && !product) {
		return <main className={className}>Товар не найден.</main>;
	}
			...
			<div className="page-shell">
				<header className="page-header">
					<div>
						...
						<h1>{isAdding ? 'Добавить товар' : 'Редактировать товар'}</h1>
					</div>
				</header>
				...
				<form className="product-form" onSubmit={handleSubmit}>
						...
						<label>
							...
							<select
								...
								defaultValue={
									product?.stock === 0
										? 'out'
										: product?.isFeatured || product?.bestseller
											? 'featured'
											: 'regular'
								}
							>
					...
					<div className="actions">
						...
						<button type="submit" className="submit-button">
							{isAdding ? 'Сохранить' : 'Сохранить изменения'}
						</button>
					</div>
				</form>
			</div>
...
export const ModeratorProductForm = styled(ModeratorProductFormContainer)`
